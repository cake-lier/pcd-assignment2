...
object StationNameParser {
  /** Parses a JSON-formatted [[String]] into a [[String]] which represents a [[it.unibo.pcd.assignment2.eventdriven.model
   *  .Station]] name.
   *
   ...
   */
  def apply(json: String): String = Json.parse(json).as[List[JsValue]].headOption.map(o => (o \ "id").as[String]).getOrElse("")
}
